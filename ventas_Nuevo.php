...
if( isset( $_POST['inIdFactura']) && !empty( $_POST['inIdFactura']) &&
    ...
    isset( $_POST['inCantidad']) && !empty( $_POST['inCantidad']) )
    {
        ...
        */
        ...
        $stmt = $base_de_datos->prepare("SELECT CASE WHEN ? <= ProExistencia THEN '1' ELSE '0' END FROM producto WHERE ProCodigo = ?;");
        $stmt->execute([$inCantidad, $inCodigo]);
        $resultValidacion = $stmt->fetchColumn();
        ...
        {
            echo '<script>alert("La cantidad solicitada es mayor que la cantidad Disponible en stock"); window.location.href="ventas.php";</script>';  
        }   
